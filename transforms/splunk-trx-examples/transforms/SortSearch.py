...
splunk_example_sort_input_setting = TransformSetting(
        name="splunk_example_sort_input",
        display_name="Splunk field which search results should be sorted by",
        setting_type="string",
        popup=True
)


@registry.register_transform(display_name="Sort Search",
                             input_entity=Phrase,
                             description="Retrieve results sorted by a given field in Splunk.",
                             output_entities=[Phrase],
                             settings=[token_global_setting, splunk_example_sort_input_setting])
class SortSearch(DiscoverableTransform):
    ...
    def create_entities(cls, request: MaltegoMsg, response: MaltegoTransform):
        try:
            ...
            splunk_sort_field_name = request.getTransformSetting(splunk_example_sort_input_setting.name)

            if not splunk_sort_field_name:
                message = "Missing 'splunk sort file name' in transform settings"
